...
\renewcommand{\nipajinTextPlayer}{%
	...
	\mysection[labelTaskresolution]{Task resolution}
		...
		\noindent
		The task succeeds if the total equals or exceeds a \keyword{target number} (\TN), set by the game master based on its difficulty. There is no automatic failure on a total of one, but that will rarely be sufficient.
		...
		When all dice of a character are exhausted, he can't do any more tasks. The character has to \keyword{take a deep breath} and pause or a short period of time, as defined by the game master. After that, the dice become available again and are put back into the upper right area.
