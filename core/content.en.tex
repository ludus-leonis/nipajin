...
\renewcommand{\nipajinTextPlayer}{%
	...
	\mysection[labelTaskresolution]{Task resolution}
		...
		\noindent
		As long as everyone agrees on the outcome, the plot moves along freely between players and game master. When the outcome of an \keyword{action} is unclear, the player picks an \keyword{available die} from the upper right area of his character sheet and rolls it. On a natural one the attempt is an \keyword{automatic failure}. Otherwise a \keyword{modifier} is added to the roll, derived from the character's background by the game master.

		\nipajinTableModifier

		\noindent
		The action succeeds if the total equals or exceeds a \keyword{target number} (\TN) set by the game master based on the difficulty of the task. There is no automatic failure on a total of one, but that will rarely be sufficient.
	...
	\mysection[labelConflict]{Conflicts}
		...
		An attack succeeds if the aggressor's \AD\ exceeds the defender's \RD. On a natural one the attack or defence fails automatically. A successful attack results in a \keyword{wound} and reduces the enemie's \HD\ by 1. Non-violent actions also help to overcome enemies. If successful, they result in \keyword{trauma}, which is represented by game tokens placed on the character sheet.
		...
		If the \HD~of a character drops below 1, or if the number of trauma tokens reach its current value, the character is overcome (dead, intimidated,~\ldots).
	...
	\mysection[labelHeal]{Healing}
		...
		\noindent
		After each \keyword{full night's rest} all player's dice become available again. Character's wounds may also heal. Their players roll the \HD\ -- if the result is higher than the original value, it becomes the new value, otherwise the \HD\ remains unchanged.
