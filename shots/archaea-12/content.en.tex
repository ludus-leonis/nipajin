...
\renewcommand{\archaeaVersion}{v0.1}
...
\renewcommand{\archaeaTranslation}{nobody}
\renewcommand{\archaeaDisclaimer}{(logos/brands excluded)}

% --- main texts ---------------------------------------------------------------

\renewcommand{\archaeaPlayers}{%
	A scenario for three to five characters.
}
\renewcommand{\archaeaSummary}{%
	NIP'AJIN Shots are short pen'n'paper role playing game scenarios. They are designed to be played in 1 or 2 hours with little or no preparation.

	To play the following scenario, you'll need the NIP'AJIN rules booklet. It is available for free at~\ldots
}
...
\renewcommand{\archaeaText}{%
	{\itshape
		It is the year 2184. Mankind has reached for the stars after inventing the jump drive. But for a long time it seemed like we were alone in our vast galaxy.

		In the Placebo sub-sector, months of space travel away from Earth, the ring-shaped mobile space station Archaea-12 and its team of 20 scientists float above newly discovered Heures-3. This planet with its flora and fauna resembles Earth 100 million years ago, even reptiles were found living down there. Tough prohibited by strong quarantine rules, scientists bring samples of plants, lizards and reptile eggs on board to study them.

		A few days ago a creature hatched from one of the football-sized eggs in Lab II. It showed an astonishing hunger for fellow test subjects and grew rapidly. Lab II was sealed off to bring the situation under control. But the clever beast found its way over the air vents into the rest of the station and developed a taste for yummy humans. In shady corners of the space station it attacked isolated scientists and always left a bloodbath. Even worse, it seems that this creature can reproduce itself and started to lay eggs.

		The crew of Archaea-12 is reduced to a few people by now -- you. Some fled using the only shuttle, the others are dead. You are trapped on board. Your only chance: find that creature and dispose of it~\ldots
	}
	...
	\mysection{Setting}
		...
		\keyword{Housing unit}: Contains the mass and a comfy living room, multiple smaller rooms with beds for 1 or 2 scientists, a small medical station as well as the bathrooms.
	...
	\mysection{Development}
		...
		\keyword{Panic phases} are resolved like conflicts while the creature attacks the PCs. Running around the station requires an \AD~and Agility\TN4. A success allows to move two spaces, a failure only one. A space is each station as well as each corridor in between.

		The creature can only be beaten with special arrangements. Regular attacks, even with the Sergeant's gun, reduce the \HD\ as usual, but when the creature drops below 1 it is only \keyword{temporarily overcome} and flees.

		During \keyword{calm phases} the creature rests somewhere for an hour, regenerates one point of its \HD\ every 10 minutes and lays more eggs. During that time the SC can use \emph{long-term activities} (\TN~Players*3, 10min/round) to build traps, deadly devices and such.

		Only by using clever traps the creature can be \keyword{overcome permanently}. But even this takes two different attempts. After the first successful attempt the creature's \HD~becomes a d4, but it can flee once more (runs away screaming, cramp onto the hull if tossed into space, ...) just to trigger another panic phase right after that. If the creature is overcome a second time using a long-term activity, it is beaten.
